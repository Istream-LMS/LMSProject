...
@Override
    public List<BigDecimal> reteriveAmt(Long loanId){
    	ResidualAmtMapper mapper=new ResidualAmtMapper();
    	return jdbcTemplate.query("select c.amount as amount  from m_loan_charge c  where c.loan_id=" + loanId + " and c.charge_time_enum=10", mapper);
    }
    private static final class ResidualAmtMapper implements RowMapper<BigDecimal>{
