...
public class Loan extends AbstractPersistable<Long> {
	...
	public void setLoanRepaymentScheduleDetail(
    ...
    //changes made by madhav
    //closing the loan with out api call throught current date
    public void closeAsMarkedForRescheduleThroughCurrentdate(final Long loanId, final LoanLifecycleStateMachine loanLifecycleStateMachine) {
        final LocalDate rescheduledOn = new LocalDate();
        final LoanStatus statusEnum = loanLifecycleStateMachine.transition(LoanEvent.LOAN_RESCHEDULE, LoanStatus.fromInt(this.loanStatus));
        if (!statusEnum.hasStateOf(LoanStatus.fromInt(this.loanStatus))) {
            this.loanStatus = statusEnum.getValue(); 
        }
