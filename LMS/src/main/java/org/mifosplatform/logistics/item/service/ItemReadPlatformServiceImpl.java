...
public class ItemReadPlatformServiceImpl implements ItemReadPlatformService{
	...
	@Autowired
	public ItemReadPlatformServiceImpl(final PlatformSecurityContext context,final RoutingDataSource dataSource) {
	...
	public List<EnumOptionData> retrieveUnitTypes() {
		final EnumOptionData gallon = UniteTypeData.UnitClassType(UnitEnumType.GALLON);
		final EnumOptionData quart = UniteTypeData.UnitClassType(UnitEnumType.QUART);
		final EnumOptionData each = UniteTypeData.UnitClassType(UnitEnumType.EACH);
		final EnumOptionData set = UniteTypeData.UnitClassType(UnitEnumType.SET);
		final EnumOptionData tube = UniteTypeData.UnitClassType(UnitEnumType.TUBE);
		final List<EnumOptionData> categotyType = Arrays.asList(gallon, quart, each, set, tube);
		return categotyType;
	}
...
private static final class SalesDataMapper implements
	...
	public String schema() {
		return " a.id as id,a.item_code as itemCode,a.item_description as itemDescription,a.item_class as itemClass,a.units as units,a.charge_code as chargeCode,round(a.unit_price,2) price,a.warranty as warranty,a.reorder_level as reorderLevel,a.manufacturer as manufacturer,"+
				"b.Used as used,b.Available as available,a.manufacturer as manufacturer," +
	...
	public ItemData mapRow(ResultSet rs, int rowNum)
		...
		return new ItemData(id,itemCode,itemDescription,itemClass,units,chargeCode,warranty,unitPrice,used,available,totalItems, reorderLevel,manufacturer);
