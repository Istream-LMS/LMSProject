...
import org.mifosplatform.logistics.grn.exception.NoGrnIdFoundException;
...
public class ItemDetailsWritePlatformServiceImp implements ItemDetailsWritePlatformService{
	...
	public ItemDetailsWritePlatformServiceImp(final ItemDetailsReadPlatformService inventoryItemDetailsReadPlatformService, 
			...
			final ItemDetailsAllocationRepository inventoryItemDetailsAllocationRepository, 
			final ItemDetailsRepository inventoryItemDetailsRepository,final FromJsonHelper fromJsonHelper, 
	...
	public CommandProcessingResult addItem(final JsonCommand command,Long flag) {
		...
		try{
			...
			if(inventoryGrn != null){
				...
				if(inventoryGrn.getReceivedQuantity() < inventoryGrn.getOrderdQuantity()){
					if(UnitEnumType.QUART.toString().equalsIgnoreCase(items.getUnits())){
					...
					}else if(UnitEnumType.TUBE.toString().equalsIgnoreCase(items.getUnits()) ||
								UnitEnumType.GALLON.toString().equalsIgnoreCase(items.getUnits())){
		...
		public CommandProcessingResult updateItem(Long id,JsonCommand command)
		{
	        try{
	        	...
	        	if(newQuantity != null && UnitEnumType.TUBE.toString().equalsIgnoreCase(items.getUnits()) ||
						UnitEnumType.GALLON.toString().equalsIgnoreCase(items.getUnits())){
	        	...
	        	if(UnitEnumType.QUART.toString().equalsIgnoreCase(items.getUnits()) && !oldSerilaNumber.equalsIgnoreCase(inventoryItemDetails.getSerialNumber()) &&inventoryItemDetails.getClientId()!=null){
	        	...
	        	if(UnitEnumType.QUART.toString().equalsIgnoreCase(items.getUnits()) && (!oldHardware.equalsIgnoreCase(inventoryItemDetails.getProvisioningSerialNumber()) || !oldSerilaNumber.equalsIgnoreCase(inventoryItemDetails.getSerialNumber())) 
		...
		private ItemDetails ItemretrieveById(Long id) {
			...
			ItemDetails itemId=this.inventoryItemDetailsRepository.findOne(id);
	              if (itemId== null) { throw new NoGrnIdFoundException(id.toString()); }
		          return itemId;	
		}
